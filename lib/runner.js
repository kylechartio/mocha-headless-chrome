...
// instead of using page.waitForFunction, we manually poll
// and use evaluate, because we ran into issues with waitForFunction
// timing out even when the result was available on the page.
async function pollForMochaResult(timeout, page, browser) {
    const start = new Date();
    while (true) {
        await sleep(100);
        if (new Date() - start > 1000 * timeout) {
            throw new Error(`pollForMochaResult timeout after ${timeout}ms exceeded`);
        }
        const res = await page.evaluate(() => window.__mochaResult__);
        if (res) {
            browser.close()
            return res
        }
...
function handleConsole(msg) {
    ...
    Promise.all(args.map(a => a.jsonValue()))
        .then(args => {
            ...
            // https://github.com/tildedave/mocha-headless-chrome/commit/397ae9
            !isStdout && msg && (msg += '\n');
            ...
            .then(browser => browser.pages()
                ...
                .then(page => {
                    ...
                    return page.evaluateOnNewDocument(initMocha, reporter)
                        .then(() => page.goto(url))
                        .then(() => pollForMochaResult(timeout, page, browser))
                }));
